...
func _ready():
	...
	call_deferred("assign_world_camera") #only call when everything is done

func queue_free(): #overwriting to prevent errors with timing with loading
...
func _physics_process(delta):
	...
	match state:
		...
		WALL_SLIDE:
			...
			if wall_axis !=0:
				sprite.scale.x = wall_axis #scaling based on the wall
...
func assign_world_camera():
		cameraFollow.remote_path = MainInstances.WorldCamera.get_path() #setting the remote path manualy
...
func fire_missile():
	...
	motion -= missile.velocity * 0.25  #this is the player motion and pushes the player the oposite direction the missile is going
...
func jump_check():
	if is_on_floor() or coyoteJumpTimer.time_left >0:
		...
		if Input.is_action_just_pressed("ui_up") and double_jump == true: #double jump code
			jump(JUMP_FORCE * .75)
			double_jump = false
...
func update_animations(input_vector):
	...
	if facing != 0:
		sprite.scale.x = facing #has to be outside loop to be all the time
	if input_vector.x != 0:
...
func get_wall_axis():#this checks to the right of us or the left of us for a wall
	var is_right_wall  = test_move(transform, Vector2.RIGHT) #returns true for a collision on right
...
func wall_slide_drop(delta): 
	var max_slide_speed = WALL_SLIDE_SPEED #sliding faster
	if Input.is_action_pressed("ui_down"):
		max_slide_speed = MAX_WALL_SLIDE_SPEED
	motion.y = min(motion.y + GRAVITY * delta, max_slide_speed) #capping with max_slide_speed

	
func wall_detach(delta,wall_axis ): #dropping off with left or right
	if Input.is_action_just_pressed("ui_right"):
		motion.x = ACCELERATION * delta #delta is for using acceleration without huge numbers
		state = MOVE
	...
	if wall_axis == 0 or is_on_floor(): #detaching if there is no walls to either side 
		state = MOVE
...
func _on_PowerUpDetector_area_entered(area):
	if area is Powerup: #checking for class name
		area._pickup()
