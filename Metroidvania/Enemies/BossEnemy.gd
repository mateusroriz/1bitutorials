...
func _ready():
	if SaverAndLoader.custom_data.boss_defeated:
		queue_free() #destroying the boss if there is a save file that he died
...
func chase_player(delta):
	...
	if player != null:
		...
		rotation_degrees = lerp(rotation_degrees, (motion.x/MAX_SPEED) *10, 0.3) #tilting the boss a bit
		
		if ((rightWallCheck.is_colliding() and motion.x >0) or 
		(leftWallCheck.is_colliding() and motion.x <0)):
			motion.x *= -0.5 #if we hit a wall go other way 
